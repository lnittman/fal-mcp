...
describe("Real-World Workflows", () => {
  ...
  describe("E-commerce Product Processing", () => {
    it("should handle batch product photo processing workflow", async () => {
      ...
      expect(bgRemovalResult.content[0].text).toMatch(
        /processed \d+ images|No image files found|Error|Directory not found/
      );
      ...
      expect(styleResult.content[0].text).toMatch(
        /processed \d+ images|No image files found|Error|Directory not found/
      );
    });
  });
  ...
  describe("AI Model Discovery Workflow", () => {
    it("should guide users through model discovery process", async () => {
      ...
      expect(docsResult.content[0].text).toMatch(
        /Parameters|No documentation available|No specific documentation/
      );
