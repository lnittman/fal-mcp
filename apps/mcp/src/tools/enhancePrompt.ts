...
export const schema = {
  ...
  model: z
    .string()
    .optional()
    .describe("Target model (e.g., 'fal-ai/flux/dev') for model-specific optimization"),
  type: z
    .enum(["image", "video", "audio"])
    .optional()
    .describe("Media type for context-aware enhancement"),
  preset: z
    .string()
    .optional()
    .describe("Optional style preset (e.g., 'pixel-art', 'photography', 'anime')"),
};
...
export default async function enhancePrompt(params: InferSchema<typeof schema>) {
  ...
  try {
    ...
    const mediaType =
      type || (model?.includes("video") ? "video" : model?.includes("audio") ? "audio" : "image");
