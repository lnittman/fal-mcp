...
describe("tool-base utilities", () => {
  ...
  describe("submitToFal", () => {
    it("should return mock response in mock mode", async () => {
      const result = await submitToFal("fal-ai/flux/dev", { prompt: "test" }, "test-tool");
    ...
    it("should return appropriate mock for video models", async () => {
      const result = await submitToFal("fal-ai/kling-video/v2", { prompt: "test" }, "test-tool");
    ...
    it("should return appropriate mock for audio models", async () => {
      const result = await submitToFal("fal-ai/stable-audio", { prompt: "test" }, "test-tool");
  ...
  describe("formatMediaResult", () => {
    it("should format URL as MCP response", () => {
      ...
      expect(result).toEqual({
        content: [{ type: "text", text: "https://example.com/media.png" }],
      });
    });
  });
  ...
  describe("validateModel", () => {
    ...
    it("should reject invalid model IDs", async () => {
      await expect(validateModel("invalid-model", "test")).rejects.toThrow(
